/**
 * XP Service
 * Manages XP awards, level calculations, and profile updates
 */
...
class XPService {
  ...
  private readonly HISTORY_LIMIT = 50; // Keep last 50 entries
  ...
  public awardXP(
    ...
    const oldRank = profile.rankTitle;
    ...
    profile.history.unshift(entry); // Add to beginning
    profile.history = profile.history.slice(0, this.HISTORY_LIMIT); // Keep only recent
  ...
  /**
   * Get XP profile for a user (or create default)
   */
  ...
  /**
   * Check if user leveled up (useful for notifications)
   */
  ...
  /**
   * Get level up notification if applicable
   */
  ...
  /**
   * Private: Get max XP for a level (for exponential scaling)
   */
  private getMaxXPForLevel(level: number): number {
    if (level <= 5) {
      // Linear up to level 5
      return LEVEL_THRESHOLDS[level] ?? 0;
    }
    // Exponential after level 5
