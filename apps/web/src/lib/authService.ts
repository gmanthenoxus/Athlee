...
export class AuthService {
  ...
  async login(email: string): Promise<User> {
    ...
    if (!user) {
      console.error(
        `❌ LOGIN FAILED: Email "${email}" not found in ${allUsers.length} users`
      );
...
/**
 * Get auth system diagnostics for debugging
 * Useful when troubleshooting login issues with ecosystem users
 */
export function getAuthDiagnostics(): {
  ...
  const message =
    allUsers.length === 0
      ? '❌ No users in system'
      : validEmailCount === 0
        ? '❌ No users have valid email'
        : validEmailCount < allUsers.length
          ? `⚠️ Only ${validEmailCount}/${allUsers.length} users have email`
          : `✅ All ${allUsers.length} users have valid email`;
